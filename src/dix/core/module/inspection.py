...
def inspect_module(root: Path, *, module_id: str) -> ModuleInspection:
    ...
    if not contract_definitions and not composition_definitions and not application_definitions:
...
def _discover_module_roots(directory: Path, trusted_root: Path) -> tuple[Path, ...]:
    ...
    if any((directory / family).is_dir() for family in ("contracts", "compositions", "apps")):
        for family in ("contracts", "compositions", "apps"):
